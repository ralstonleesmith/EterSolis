    ...
    <>
      ...
      <section className="section-padding bg-mist dark:bg-black">
        <div className="container-shell grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {kymnisPillars.map((pillar) => (
            <article key={pillar.title} className="surface-lift rounded-lg border border-coal/10 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-white/5">
              ...
              <p className="mt-3 leading-7 text-coal dark:text-on-dark-muted">{pillar.description}</p>
            </article>
          ))}
        </div>
      </section>
      <section className="section-padding">
        <div className="container-shell grid gap-10 lg:grid-cols-[0.85fr_1.15fr] lg:items-start">
          <div>
            <p className="text-xs font-black uppercase tracking-normal text-subtle dark:text-sunshine">Operating pathway</p>
            <h2 className="mt-4 text-4xl font-black tracking-normal text-carbon dark:text-white">From impact context to responsible action.</h2>
          ...
          <div className="grid gap-4">
            {kymnisFlow.map((step, index) => (
              <div key={step} className="rounded-lg border border-coal/10 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-white/5">
                <span className="text-xs font-black uppercase tracking-normal text-subtle dark:text-sunshine">Step {index + 1}</span>
      ...
      <section className="section-padding bg-mist dark:bg-black kymnis-glow">
        <div className="container-shell grid gap-10 lg:grid-cols-[0.9fr_1.1fr]">
          <div>
            ...
            <h2 className="mt-4 text-4xl font-black tracking-normal text-carbon dark:text-white">A user should feel like they are showing a problem and asking for help.</h2>
            <p className="mt-4 leading-8 text-coal dark:text-on-dark-muted">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {kymnisAppButtons.map((button) => (
              <article key={button.label} className="surface-card rounded-lg p-5">
                <h3 className="text-xl font-black">{button.label}</h3>
                <p className="mt-3 leading-7 text-coal dark:text-on-dark-muted">{button.description}</p>
              </article>
            ))}
          </div>
        </div>
      ...
      <section className="section-padding">
        <div className="container-shell rounded-lg border border-coal/10 bg-white p-7 shadow-sm dark:border-white/10 dark:bg-white/5">
          ...
          <h2 className="mt-4 text-4xl font-black tracking-normal text-carbon dark:text-white">{kymnisBrand.essence}</h2>
          <div className="mt-8 grid gap-3 md:grid-cols-3">
            {kymnisUserJourney.map((step) => (
              <div key={step} className="rounded-lg border border-coal/10 bg-cool p-4 font-black text-carbon dark:border-white/10 dark:bg-black/40 dark:text-white">
                {step}
              </div>
            ))}
          </div>
