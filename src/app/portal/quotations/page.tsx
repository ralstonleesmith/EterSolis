...
export const metadata: Metadata = { title: 'Portal Quotations | EterSolis' };

const rows = [
  ['ES-Q-DEMO-0001', 'Assessment and material classification', 'ZAR 3,450.00', 'Internal review'],
  ['ES-Q-DEMO-0002', 'Pickup and receiving scope', 'ZAR 6,900.00', 'Draft']
];

export default function PortalQuotationsPage() {
  return <CommercialTable title="Quotations" eyebrow="Documentation before payment" rows={rows} columns={['Quotation', 'Scope', 'Total', 'Status']} note="A quotation documents scope, assumptions, exclusions and validity before invoice or payment." />;
}

function CommercialTable({ title, eyebrow, rows, columns, note }: { title: string; eyebrow: string; rows: string[][]; columns: string[]; note: string }) {
  return (
