...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-coal/10 bg-white/92 shadow-[0_10px_40px_rgba(0,0,0,0.04)] backdrop-blur-xl dark:border-white/10 dark:bg-black/88">
      <div className="container-shell flex min-h-[78px] items-center justify-between gap-6">
        ...
        <nav className="hidden items-center gap-4 text-sm font-bold text-coal lg:flex xl:gap-6 dark:text-white/86" aria-label="Primary navigation">
          {navItems.map((item) => (
            <Link
              key={item.href}
              href={item.href}
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          <ThemeToggle />
          <Link href="/helios" className="hidden rounded-full border border-coal/10 bg-white/70 px-4 py-3 text-sm font-black text-coal transition hover:-translate-y-0.5 hover:border-sunshine hover:text-carbon dark:border-white/10 dark:bg-white/6 dark:text-white dark:hover:text-sunshine md:inline-flex">
            Talk to Helios
          </Link>
