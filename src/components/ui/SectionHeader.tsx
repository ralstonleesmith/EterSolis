export function SectionHeader({ eyebrow, title, description, invert = false }: { eyebrow: string; title: string; description?: string; invert?: boolean }) {
  return (
    <div className="max-w-3xl">
      <p className={`text-xs font-black uppercase tracking-[0.28em] ${invert ? 'text-sunshine' : 'text-coal/70'}`}>{eyebrow}</p>
      <h2 className={`mt-4 text-4xl font-black leading-tight tracking-[-0.04em] md:text-6xl ${invert ? 'text-white' : 'text-carbon'}`}>{title}</h2>
